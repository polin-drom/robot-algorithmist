...
"""
    count_moves!(r::Robot, side::HorizonSide)

-- Перемещает Робота в заданном направлении до упора и возвращает сделанное число шагов    
"""
function count_moves!(r::Robot, side::HorizonSide)
    num_steps = 0
    while move_if_possible!(r, side) > -1
        num_steps += 1
    end
    return num_steps
end

function count_moves!(r::Robot, side::HorizonSide, steps::Int)
    num_steps = 0
    walls = 0
    while steps > 0
        walls += move_if_possible!(r, side)
        num_steps += 1
        steps -= 1
    end
    return num_steps - walls
end
...
function move_if_possible!(r::Robot, now_side::HorizonSide)::Int
...
-- Перемещает Робота в заданном направлении до упора и расставляет маркеры
"""
